...
if __name__ == '__main__':
    ...
    placeholder_text = "Vardas Pavardė, Vardas Vardas Pavardė"
    authors_list_var.set(placeholder_text)
    ...
    autorius_laukelis = ttk.Entry(frm, textvariable=authors_list_var)

    self_citation_button = ttk.Button(frm, text="Įvertinti savęs citavimą", command=lambda: evaluate_self_citation())
    ...
    def change_file_selection_buttons_action():
        ...
        if is_thread_running == 1:
            tkinter.messagebox.showerror(title="Vyksta analizė",
                                         message="Kol failas yra analizuojamas, kito failo pasirinkti negalima")
        else:
            open_file()
    ...
    def selected_file_UI():
        show_selected_file()
        autorius_label.grid(row=2, column=0, sticky=EW)
        autorius_laukelis.grid(row=2, column=1, sticky=EW, padx=10)
        self_citation_button.grid(row=2, column=2)

        saves_citavimas_checkbox.grid(row=3, column=1, padx=20)

        struktura_label.grid(row=4, column=0, sticky=EW)
        ...
        references_length_label.grid(row=11, column=0, pady=5, sticky=E)
        references_length_textbox.grid(row=11, column=1, pady=5)
        citation_label.grid(row=12, column=0, pady=5, sticky=E)
    ...
    def hide_selected_UI():
        ...
        self_citation_button.grid_forget()
    ...
    def evaluate_self_citation():
        special_characters = '"!@#$%^&*()-+?_=<>/";'
        global authors_list_var, self_citation_var, placeholder_text

        author_list = authors_list_var.get()
        if author_list == placeholder_text:
            tkinter.messagebox.showerror(title="Autorių sąrašas", message="Nenurodytas autorių sąrašas")
        if any(c in special_characters for c in author_list):
            tkinter.messagebox.showerror(title="Autorių sąrašas", message="Autorius skirkite naudodami kablelį (,)")

        check_self_citation = con.check_self_citation(author_list)
        if check_self_citation == 0:
            tkinter.messagebox.showinfo(title="Savęs citavimas", message="Savęs citavimas nebuvo rastas")
        if check_self_citation == 1:
            tkinter.messagebox.showinfo(title="Savęs citavimas", message="Savęs citavimas buvo rastas")
        self_citation_var.set(check_self_citation)
